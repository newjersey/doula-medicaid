...
Additionally, we currently have tech debt in the form of a discrepancy between staging and
production deployments. For production, state firewalls are still redirecting to a hardcoded DNS of
the VPC-internal application load balancer, that was randomly generated by cloud formation. For
staging on-prem DNS servers point to the stable CNAME of the application load balancer, as
configured by the private hosted zone within the VPC. This thus means that we need to specify the
IPs of these on-prem DNS servers in staging. However, we have not configured or figured out what the
equivalent would be for production. Staging deployment thus requires additional cdk context value
`dnsIps`, even though production does not.

### First-time HTTP deployment

First, deploy the cdk stack with a http internal ALB, replacing `<profile name>` and env
`<production or staging>`. For `dnsIps`, replace with a comma-separated list of IPs of on-prem DNS
servers.
...
# For staging
npm run cdk:deploy -- --profile <profile name> --context env=staging --context dnsIps=<comma-separated list of IPs of on-prem DNS servers>
```
...
# For staging
npm run cdk:deploy -- --profile <profile name> --context env=<production or staging> --context https=true --context certificateArn=<certificate arn> --context dnsIps=<comma-separated list of IPs of on-prem DNS servers>
```
