...
import type { DisclosureData } from "@/app/form/(formSteps)/disclosures/DisclosureData";
...
const DisclosuresStep1: React.FC = () => {
  ...
  const { register, handleSubmit, watch } = useForm<DisclosureData>({
    defaultValues: {
  ...
  const onSubmit: SubmitHandler<DisclosureData> = (data) => {
    let key: keyof DisclosureData;
    for (key in data) {
