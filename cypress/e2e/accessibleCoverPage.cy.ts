 ...
 *
 * This test has quite a few exceptions to our usual patterns.
 *
 ...
 *    do so.
 * 2. Additionally, pdfjs-dist can only read documents from URLs, and not local files. So we have
...
it("should download the application with a cover page that has accessibly readable text [accessibleCoverPage]", () => {
  ...
  cy.exec(`cp "${downloadedPdfpath}" "public/${filePathInPublicDir}"`, {
     ...
     * works in that the test passes, but also in that in manual attempts, it fails when it should,
     * both locally and on CI. E.g., test the failure by changing `expectedCoverPageText` to
     * something invalid
     *
     ...
     * though `cy.exec()` is then-able, it's not actually a promise, and you can't await them. Per
     * [cypress docs](https://docs.cypress.io/app/core-concepts/variables-and-aliases), the commands
     * outside of the .then() will not run until all of the nested commands finish.
     *
