...
credentials_nested = list(df["credential"])
all_credentials = []
for credentials in credentials_nested:
    if not isinstance(credentials, float):
        for space_split_credentials in credentials.split(", "):
            all_credentials += space_split_credentials.split(",")

srs = pd.Series(collections.Counter(all_credentials))
srs.iloc[srs.index.str.lower().argsort()]
